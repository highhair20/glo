 ...
 * http://gloframework.com/license/new-bsd
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to license@gloframework.com so we can send you a copy immediately.
 *
 * @category   Glo
 * @package    Glo_Tool
 * @subpackage Framework
 * @copyright  Copyright (c) Glo Framework
 * @license    http://gloframework.com/license/new-bsd     New BSD License
 * @version    $Id$
 */
...
class Glo
{
    ...
    protected function _detectHomeDirectory($mustExist = true, $returnMessages = true)
    {
        ...
        $homeDirectory = getenv('GLO_HOME'); // check env var GLO_HOME
        if ($homeDirectory) {
            $this->_logMessage('Home directory found in environment variable GLO_HOME with value ' . $homeDirectory, $returnMessages);
    ...
    protected function _detectStorageDirectory($mustExist = true, $returnMessages = true)
    {
        ...
        $storageDirectory = getenv('GLO_STORAGE_DIR');
        if ($storageDirectory) {
            $this->_logMessage('Storage directory path found in environment variable GLO_STORAGE_DIR with value ' . $storageDirectory, $returnMessages);
        ...
        if ($homeDirectory) {
            $storageDirectory = $homeDirectory . '/.glo/';
    ...
    protected function _detectConfigFile($mustExist = true, $returnMessages = true)
    {
        ...
        $configFile = getenv('GLO_CONFIG_FILE');
        if ($configFile) {
            $this->_logMessage('Config file found environment variable GLO_CONFIG_FILE at ' . $configFile, $returnMessages);
        ...
        $homeDirectory = ($this->_homeDirectory) ? $this->_homeDirectory : $this->_detectHomeDirectory(true, false);
        if ($homeDirectory) {
            $configFile = $homeDirectory . '/.glo.ini';
        ...
        if ($storageDirectory) {
            $configFile = $storageDirectory . '/glo.ini';
    ...
    protected function _setupPHPRuntime()
    {
        ...
        $gloINISettings = parse_ini_file($this->_configFile);
        $phpINISettings = ini_get_all();
        foreach ($gloINISettings as $gloINIKey => $gloINIValue) {
            if (substr($gloINIKey, 0, 4) === 'php.') {
                $phpINIKey = substr($gloINIKey, 4);
                if (array_key_exists($phpINIKey, $phpINISettings)) {
                    ini_set($phpINIKey, $gloINIValue);
                }
            }
        }
    }
    ...
    protected function _setupToolRuntime()
    {
        ...
        // if Glo is not in the include_path, but relative to this file, put it in the include_path
        if ($includePathPrepend || $includePathFull) {
        ...
        $gloIncludePath['relativePath'] = dirname(__FILE__) . '/../library/';
        if (file_exists($gloIncludePath['relativePath'] . 'Zend/Tool/Framework/Client/Console.php')) {
            set_include_path(realpath($gloIncludePath['relativePath']) . PATH_SEPARATOR . get_include_path());
        }
...
***************************** GLO ERROR ********************************
In order to run the glo command, you need to ensure that Glo Framework
is inside your include_path.  There are a variety of ways that you can
ensure that this glo command line tool knows where the Glo Framework
library is on your system, but not all of them can be described here.

The easiest way to get the glo command running is to give it the include
path via an environment variable ZEND_TOOL_INCLUDE_PATH or
ZEND_TOOL_INCLUDE_PATH_PREPEND with the proper include path to use,
then run the command "glo --setup".  This command is designed to create
a storage location for your user, as well as create the glo.ini file
that the glo command will consult in order to run properly on your
system.

Example you would run:

$ ZEND_TOOL_INCLUDE_PATH=/path/to/library glo --setup
...
EOS;
    ...
    protected function _runInfo()
    {
        echo 'Zend_Tool & CLI Setup Information' . PHP_EOL
           . '(available via the command line "glo --info")'
           . PHP_EOL;
        ...
        echo 'To change the setup of this tool, run: "glo --setup"';
...
Important Environment Variables:
    ...
    GLO_TOOL_INCLUDE_PATH
        - set the include_path for this tool to use this value
    GLO_TOOL_INCLUDE_PATH_PREPEND
        - prepend the current php.ini include_path with this value

Search Order:
    Home Directory:
        - GLO_HOME, then HOME (*nix), then HOMEPATH (windows)
    Storage Directory:
        - GLO_STORAGE_DIR, then {home}/.glo/
    Config File:
        - GLO_CONFIG_FILE, then {home}/.glo.ini, then {home}/glo.ini,
          then {storage}/glo.ini

Commands:
    glo --setup storage-directory
        - setup the storage directory, directory will be created
    glo --setup config-file
...
EOS;
    ...
    protected function _runTool()
    {
        ...
        // ensure that glo.php loads the Zend_Tool_Project features
        $configOptions['classesToLoad'] = 'Glo_Tool_Project_Provider_Manifest';
